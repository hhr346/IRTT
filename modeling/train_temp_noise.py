...
def add_realistic_noise(X):
    """
    X: numpy array, shape (N, n_features)
    Add noise BEFORE normalization.
    """
    X = X.copy()
    # Radiances (already in brightness temperature K)
    # X[:, :n_radiance] += np.random.normal(loc=0.0, scale=0.1, size=(X.shape[0], n_radiance))
    # Surface skin temperature (~1 K)
    X[:, -2] += np.random.normal(loc=0.0, scale=1.0, size=X.shape[0])
    # Surface pressure (~30 hPa)
    # X[:, -1] += np.random.normal(loc=0.0, scale=10.0, size=X.shape[0])
    return X
...
def train_model(X: np.ndarray, y: np.ndarray):
    ...
    X_train = add_realistic_noise(X_train)
...
if __name__ == '__main__':
    ...
    filename = 'total_dataset'
    filepath = f'{OUTPUT_OUT}/{filename}.nc'

    ncfile = nc.Dataset(filepath, mode='r')
    print(f"Opening file {filepath}")
    inputs = np.asarray(ncfile.variables['input'][:, :])[:, :n_features]
